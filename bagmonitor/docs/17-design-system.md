# Design System & Digital Specs

A single source of truth for anyone creating digital materials for BagMonitor — web, social, email, presentations, or any other touchpoint.
...
- Do not add drop shadows to the mark (the gradient provides the depth)
...
The core brand color. Used for CTAs, interactive elements, highlights, and the logo gradient.
...
### Surface — Dark Gray (UI Backgrounds & Text)
...
Used for operational status indicators, positive metrics, and "live" signals.
...
The canonical dark page uses layers to create depth:
...
For primary heading highlights (used on the hero H1):
...
Recommended library: **Heroicons** (already used in components). Supplement with **Phosphor Icons** (thin/regular weight) for marketing materials.
...
|---|---|
| Hero | `pt-16 pb-32` (navbar offset + breathing room) |
...
**Glass / liquid-glass button** (hero secondary)

```
Background:   rgba(255,255,255,0.18) → rgba(255,255,255,0.10) → rgba(255,255,255,0.15)
...
For featured/highlighted cards: add `gradient-border` animated rotating border.
...
Live status dot: `8px × 8px`, `success-400` (#34D399), with `pulse-glow` animation.
...
```css
/* Standard blue glow (CTA buttons, featured elements) */
box-shadow: 0 0 20px rgba(99, 102, 241, 0.15),
            0 0 60px rgba(99, 102, 241, 0.05);

/* Strong blue glow (hero CTA hover) */
...
Backdrop blur for floating panels, navbars, and glass buttons:
...
### Gradients on dark backgrounds

Use radial gradients sparingly to add depth behind key sections:
...
| Scroll-triggered sections | 0.6s | `cubic-bezier(0.25,0.1,0.25,1)` | Framer Motion, once |
...
- Above-the-fold LCP elements must render immediately visible — no opacity:0 initial state

---

## Digital Materials Guide

### Social Media

| Platform | Size | Notes |
|---|---|---|
| LinkedIn post | 1200 × 628px | Use `surface-950` bg, single stat or quote |
...
**Recommended layout pattern for stat cards:**
...
Avoid using `backdrop-filter` or CSS `clip-path` in email — fallback to flat colors.

### Presentations (Pitch Deck / Sales Deck)
...
| Data callouts | Display type with gradient text, surrounded by subtle radial glow |
| Charts | Use primary-400/500 for main series, success-400 for positive, danger-400 for negative |

### Print

When adapting to print or light backgrounds:
...
| Apply gradient text to full paragraphs | Limit gradient text to 1–5 words in headings |
...
| Use success green for decorative purposes | Reserve green for live / operational status only |
